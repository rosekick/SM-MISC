...
void ForceCheapTrails(int entity)
{
    float fOrigin[3];
    GetClientAbsOrigin(entity, fOrigin);
    
    gF_PlayerOrigin[entity][0] = fOrigin[0];
    gF_PlayerOrigin[entity][1] = fOrigin[1];
    gF_PlayerOrigin[entity][2] = fOrigin[2];
    
    CreatePlayerTrail(entity, gF_PlayerOrigin[entity]);
}

void CreatePlayerTrail(int client, float origin[3])
{
	if(gI_SelectedTrail[client] == TRAIL_NONE || !IsPlayerAlive(client))
	{
		return;
	}
	...
	TE_SetupBeamPoints(fFirstPos, fSecondPos, gI_BeamSprite, 0, 0, 0, 1.0, 3.0, 3.0, 10, 0.0, color, 0);
}
        ...
        if(StrContains(classname, "_projectile", false) != -1)
        {
            ...
            if(0 < client <= MaxClients)
            {
                ...
                if(gI_TrailSettings[choice].iSpecialColor == 1)
                {
                    ...
                    gF_LastPosition[entity][0] = origin[0];
                    gF_LastPosition[entity][1] = origin[1];
                    gF_LastPosition[entity][2] = origin[2];
                }
            }
        }
    }
